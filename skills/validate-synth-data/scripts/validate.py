...
def check_structural(datasets: Path) -> list[CheckResult]:
    ...
    _check(r, "DM: RFENDTC present and = RFPENDTC (CG0142)",
           "RFENDTC" in dm.columns and (dm["RFENDTC"] == dm["RFPENDTC"]).all())
    _check(r, "DM: RFICDTC non-empty",
           (dm["RFICDTC"].str.strip() != "").all() if "RFICDTC" in dm.columns
           else False, "" if "RFICDTC" in dm.columns else "column missing")
    _check(r, "DM: RFPENDTC non-empty",
           (dm["RFPENDTC"].str.strip() != "").all() if "RFPENDTC" in dm.columns
           else False, "" if "RFPENDTC" in dm.columns else "column missing")

    # EX
    ex = _load(datasets, "EX")
    _check(r, "EX: EXTRT ∈ {FULVESTRANT, EVEROLIMUS, PLACEBO}",
    ...
    _check(r, "EX: EXDOSE > 0 for non-PLACEBO records",
           bad_nonplc == 0, f"{bad_nonplc} bad values")
    _check(r, "EX: EXROUTE ∈ {INTRAMUSCULAR, ORAL}",
           ex["EXROUTE"].isin({"INTRAMUSCULAR", "ORAL"}).all())
    _check(r, "EX: EXSTDY and EXENDY present",
    ...
    _check(r, "EX: Fulvestrant C1D1 and C1D15 doses = 250 mg",
           bad_loading == 0, f"{bad_loading} records with wrong loading dose")
    # CG0102: EXTRT=PLACEBO must have EXDOSE=0
    plc = ex[ex["EXTRT"] == "PLACEBO"]
    bad_plc_dose = (~(_num(plc["EXDOSE"]) == 0)).sum()
    _check(r, "EX: PLACEBO EXDOSE = 0 (CG0102)",
           bad_plc_dose == 0, f"{bad_plc_dose} records with non-zero placebo dose")

    # TR
    tr = _load(datasets, "TR")
    _check(r, "TR: TRTESTCD ∈ {LDIAM, SUMDIAM}",
           tr["TRTESTCD"].isin({"LDIAM", "SUMDIAM"}).all())
    _check(r, "TR: TRSTRESU = mm", (tr["TRSTRESU"] == "mm").all())
    _check(r, "TR: LOBXFL ∈ {Y, ''}",
           tr["LOBXFL"].isin({"Y", ""}).all())
    _check(r, "TR: TRLNKID present (not TRLINKID)",
           "TRLNKID" in tr.columns and "TRLINKID" not in tr.columns)
    _check(r, "TR: EPOCH ∈ {INDUCTION, CONTINUATION} (FB2201)",
           "EPOCH" in tr.columns and tr["EPOCH"].isin({"INDUCTION", "CONTINUATION"}).all())
    _check(r, "TR: TRMETHOD = CT SCAN present", "TRMETHOD" in tr.columns and
           (tr["TRMETHOD"] == "CT SCAN").all())
    _check(r, "TR: TREVAL = INVESTIGATOR present", "TREVAL" in tr.columns and
           (tr["TREVAL"] == "INVESTIGATOR").all())
    ...
    _check(r, "TR: SUMDIAM records present (RECIST supplement)",
           len(sumdiam) > 0)
    if len(sumdiam):
        bl_sumd = sumdiam[sumdiam["LOBXFL"] == "Y"]
        _check(r, "TR: exactly one baseline SUMDIAM per subject",
               bl_sumd.groupby("USUBJID").size().eq(1).all())
        _check(r, "TR: SUMDIAM TRGRPID = TARGET",
               (sumdiam["TRGRPID"] == "TARGET").all())
    # TRSTRESN ≥ 5 mm for evaluable LDIAM records (NOT DONE records may be empty)
    ldiam_eval = tr[(tr["TRTESTCD"] == "LDIAM") & (tr.get("TRSTAT", pd.Series([""])) != "NOT DONE")] \
        if "TRSTAT" in tr.columns else tr[tr["TRTESTCD"] == "LDIAM"]
    # Allow 0 (absent lesion) or ≥ 5 mm
    ldiam_num = _num(ldiam_eval["TRSTRESN"])
    bad_ldiam = ((ldiam_num > 0) & (ldiam_num < 5)).sum()
    _check(r, "TR: LDIAM TRSTRESN = 0 (absent) or ≥ 5 mm for evaluable records",
           bad_ldiam == 0, f"{bad_ldiam} values between 0 and 5 mm")

    # RS
    rs = _load(datasets, "RS")
    _check(r, "RS: RSTESTCD = OVRLRESP", (rs["RSTESTCD"] == "OVRLRESP").all())
    _check(r, "RS: RSSTRESC ∈ {CR, PR, SD, PD, NE} (RECIST 1.1)",
           rs["RSSTRESC"].isin({"CR", "PR", "SD", "PD", "NE"}).all(),
           f"{(~rs['RSSTRESC'].isin({'CR','PR','SD','PD','NE'})).sum()} bad values")
    _check(r, "RS: EPOCH ∈ {INDUCTION, CONTINUATION} (FB2201)",
           "EPOCH" in rs.columns and rs["EPOCH"].isin({"INDUCTION", "CONTINUATION"}).all())
    _check(r, "RS: RSDRVFL = Y for derived records (CG0563)",
           "RSDRVFL" in rs.columns and (rs["RSDRVFL"] == "Y").all())

    # DS
    ds = _load(datasets, "DS")
    _check(r, "DS: DSCAT ∈ {PROTOCOL MILESTONE, DISPOSITION EVENT}",
           ds["DSCAT"].isin({"PROTOCOL MILESTONE", "DISPOSITION EVENT"}).all())
    _valid_dsdecod = {
        "INFORMED CONSENT OBTAINED", "RANDOMIZED",
        "PROGRESSIVE DISEASE", "WITHDRAWAL BY SUBJECT", "LOST TO FOLLOW-UP", "DEATH",
    }
    bad_decod = (~ds["DSDECOD"].isin(_valid_dsdecod)).sum()
    _check(r, "DS: DSDECOD ∈ valid controlled vocabulary",
           bad_decod == 0, f"{bad_decod} unexpected values")
    ...
    _check(r, "DS: EPOCH empty for PROTOCOL MILESTONE records (CG0073)",
           bad_pm_epoch == 0, f"{bad_pm_epoch} PROTOCOL MILESTONE records with non-empty EPOCH")
    _valid_epoch = {"", "INDUCTION", "CONTINUATION", "FOLLOW-UP"}
    bad_epoch = (~ds["EPOCH"].isin(_valid_epoch)).sum()
    _check(r, "DS: EPOCH ∈ {'' (milestone), INDUCTION, CONTINUATION, FOLLOW-UP}",
           bad_epoch == 0, f"{bad_epoch} unexpected values")
    # CG0066: DSTERM must equal DSDECOD for PROTOCOL MILESTONE records
    _check(r, "DS: DSTERM present",
    ...
    if "DSTERM" in ds.columns:
        ...
        _check(r, "DS: DSTERM = DSDECOD for PROTOCOL MILESTONE records (CG0066)",
               pm_mismatch == 0, f"{pm_mismatch} mismatches")
    ...
    if len(death_ds):
        ...
        _check(r, "DS: DEATH record DSDTC = DM DTHDTC (FB0611)",
               bad_dth == 0, f"{bad_dth} mismatches")

    # ADSL
    adsl = _load(datasets, "ADSL")
    _check(r, "ADSL: TRT01A ∈ {Treatment, Placebo}",
           adsl["TRT01A"].isin({"Treatment", "Placebo"}).all())
    _check(r, "ADSL: TRT01P present and = TRT01A (no crossover)",
           "TRT01P" in adsl.columns and (adsl["TRT01P"] == adsl["TRT01A"]).all())
    _check(r, "ADSL: CNSR ∈ {0, 1}",
           adsl["CNSR"].isin({"0", "1"}).all())
    _check(r, "ADSL: PFS > 0",
           (_num(adsl["PFS"]) > 0).all(),
           f"{(_num(adsl['PFS']) <= 0).sum()} bad values")
    for flag in ("ITTFL", "SAFFL", "PPROTFL"):
        _check(r, f"ADSL: {flag} present and ∈ {{Y, N}}",
    ...
    _check(r, "ADTTE: CNSR ∈ {0, 1}",
           adtte["CNSR"].isin({"0", "1"}).all())
    _check(r, "ADTTE: AVAL > 0",
           (_num(adtte["AVAL"]) > 0).all())
...
def check_integrity(datasets: Path) -> list[CheckResult]:
    ...
    for name, df in [("EX", ex), ("TR", tr), ("RS", rs), ("DS", ds),
                     ("ADSL", adsl), ("ADTTE", adtte), ("TU", tu)]:
        orphans = set(df["USUBJID"]) - dm_ids
        _check(r, f"{name}: all USUBJIDs present in DM", len(orphans) == 0,
               f"{len(orphans)} orphan subjects")

    missing_adsl = dm_ids - set(adsl["USUBJID"])
    _check(r, "DM: all subjects in ADSL", len(missing_adsl) == 0,
           f"{len(missing_adsl)} subjects missing from ADSL")

    missing_adtte = dm_ids - set(adtte["USUBJID"])
    _check(r, "DM: all subjects in ADTTE", len(missing_adtte) == 0,
    ...
    _check(r, "EX: EVEROLIMUS only given to TRT arm", len(wrong_ever) == 0,
           f"{len(wrong_ever)} PLC subjects received EVEROLIMUS")

    wrong_plac = placebo_users - plc_ids
    _check(r, "EX: PLACEBO only given to PLC arm", len(wrong_plac) == 0,
           f"{len(wrong_plac)} TRT subjects received PLACEBO")

    trt_missing_ever = trt_ids - everolimus_users
    _check(r, "EX: all TRT subjects have EVEROLIMUS record", len(trt_missing_ever) == 0,
           f"{len(trt_missing_ever)} TRT subjects missing EVEROLIMUS")

    plc_missing_plac = plc_ids - placebo_users
    _check(r, "EX: all PLC subjects have PLACEBO record", len(plc_missing_plac) == 0,
    ...
    _check(r, "DS: exactly 5 records per subject", bad_count == 0,
    ...
    _check(r, "DS: every subject has a STUDY PARTICIPATION end record",
           len(missing_sp) == 0, f"{len(missing_sp)} subjects missing record")
    ...
    _check(r, "ADSL: TRT01A consistent with DM ARMCD", mismatched == 0,
...
def check_recist(datasets: Path) -> list[CheckResult]:
    ...
    sumd_bl_ids   = set(tr[(tr.get("TRTESTCD", pd.Series()) == "SUMDIAM") & (tr["LOBXFL"] == "Y")]["USUBJID"]) \
                    if "TRTESTCD" in tr.columns else set(tr[tr["LOBXFL"] == "Y"]["USUBJID"])
    missing_baseline = rs_ids - sumd_bl_ids
    _check(r, "TR: every RS subject has a baseline SUMDIAM record",
    ...
    if len(tu):
        ...
        _check(r, "TU: all subjects have tumor identification records",
               len(missing_tu) == 0, f"{len(missing_tu)} subjects missing TU records")
        _check(r, "TU: TUTESTCD ∈ {TIND, NTIND, TUMIDENT}",
        ...
        _check(r, "TU: TUMIDENT TULNKID links to a TR TRLNKID",
               orphan_tu == 0, f"{orphan_tu} unlinked TU records")

    # RS dates should match a TR assessment date for that subject
    tr_date_keys = set(zip(tr["USUBJID"], tr["TRDTC"]))
    unmatched = [(u, d) for u, d in zip(rs["USUBJID"], rs["RSDTC"])
                 if (u, d) not in tr_date_keys]
    _check(r, "RS: every response date matches a TR assessment date",
           len(unmatched) == 0,
    ...
    _check(r, "RS: no subject has more than one PD assessment",
           multi_pd == 0, f"{multi_pd} subjects with repeated PD records")
    ...
    if len(sumdiam) == 0:
        _check(r, "RS: RSSTRESC consistent with RECIST v1.1 SUMDIAM thresholds", False,
        ...
        if len(common) == 0:
            ...
            _check(r, "RS: RSSTRESC consistent with RECIST v1.1 SUMDIAM thresholds",
                   mm == 0, f"{mm} mismatches out of {len(common)} checked")
...
def check_timeline_death(datasets: Path) -> list[CheckResult]:
    ...
    _check(r, "DM: RFSTDTC has variability (>10 distinct dates)", n_rfstdtc > 10,
           f"{n_rfstdtc} distinct dates")

    durations = (dt(dm["RFXENDTC"]) - dt(dm["RFXSTDTC"])).dt.days.dropna()
    dur_std = durations.std()
    _check(r, "DM: Study durations vary (stddev > 30 days)", dur_std > 30,
    ...
    _check(r, "DM: DTHDTC ≥ RFSTDTC (no death before study start)",
           bad_dth_before_rfst == 0, f"{bad_dth_before_rfst} violations")
    ...
    _check(r, "TR: no records after death (TRDTC ≤ DTHDTC)", bad_tr_dth == 0,
           f"{bad_tr_dth} records after death")

    bad_rs_dth = _after_death(rs, "RSDTC")
    _check(r, "RS: no records after death (RSDTC ≤ DTHDTC)", bad_rs_dth == 0,
           f"{bad_rs_dth} records after death")

    bad_ex_dth = _after_death(ex, "EXSTDTC")
    _check(r, "EX: no records starting after death (EXSTDTC ≤ DTHDTC)", bad_ex_dth == 0,
    ...
    _check(r, "DM/DS: DTHFL=Y ↔ DS DEATH record consistent",
    ...
    _check(r, "EX: EXSTDTC ≤ EXENDTC", bad_ex_order == 0,
    ...
    _check(r, "All domains: no dates beyond study end (2026-12-31)",
           future_dates == 0, f"{future_dates} future dates")
...
def check_population(datasets: Path) -> list[CheckResult]:
    ...
    ages = _num(dm["AGE"])
    _check(r, "DM: all ages in plausible range (18–85)",
           ((ages >= 18) & (ages <= 85)).all(),
           f"{((ages < 18) | (ages > 85)).sum()} implausible values")

    age_std = ages.std()
    _check(r, "DM: age has realistic variability (stddev > 5 years)", age_std > 5,
           f"stddev={age_std:.1f}")
    ...
    _check(r, "DM: enrollment dates span > 90 days (not all same date)", enroll_span > 90,
           f"span={enroll_span} days")

    tr_counts = tr.groupby("USUBJID")["TRSEQ"].count()
    _check(r, "TR: record counts vary across subjects (not all identical)",
           tr_counts.nunique() > 1,
           f"{tr_counts.nunique()} distinct counts (min={tr_counts.min()}, max={tr_counts.max()})")

    rs_counts = rs.groupby("USUBJID")["RSSEQ"].count()
    _check(r, "RS: record counts vary across subjects (realistic missingness)",
           rs_counts.nunique() > 1,
...
def check_duplicates_and_coverage(datasets: Path) -> list[CheckResult]:
    ...
    _check(r, "DM: no duplicate USUBJIDs", dup_dm == 0,
           f"{dup_dm} duplicates")
    ...
    for name, df, keys in [
        ...
        _check(r, f"{name}: no duplicate sequence keys (USUBJID+{keys[1]})",
    ...
    _check(r, "TR: baseline dates are not all identical (schedules vary)",
           bl_dates.nunique() > 1,
    ...
    _check(r, "DM: every subject appears in ≥1 clinical domain (EX/TR/RS)",
           len(uncovered) == 0, f"{len(uncovered)} subjects with no domain records")
    ...
    _check(r, "DM ↔ ADSL: subject sets match exactly",
           len(mismatch) == 0, f"{len(mismatch)} mismatched subjects")
...
def check_pfs_fidelity(datasets: Path) -> list[CheckResult]:
    ...
    for arm, target in TARGET_PFS.items():
        ...
        if len(subset) == 0:
            _check(r, f"PFS fidelity: {arm} arm present in ADTTE", False,
                   "no records found")
            continue
...
def build_report(datasets: Path, categories: dict[str, list[CheckResult]],
                 n_subjects: int) -> str:
    lines: list[str] = []
